...
export default function CategoriesPage() {
  ...
  async function moveCategory(index, offset) {
    ...
    if (reordering || newIndex < 0 || newIndex >= categories.length) return;

    const previousCategories = categories;
    const reordered = [...categories];
    const [category] = reordered.splice(index, 1);
    reordered.splice(newIndex, 0, category);
    const prioritized = reordered.map((item, position) => ({ ...item, priority: position + 1 }));

    setCategories(prioritized);
    setOrderError('');
    setReordering(true);
    try {
      const res = await api.put('/categories/order', {
        category_ids: prioritized.map((item) => item.id),
      });
      setCategories(res.data.data.categories || prioritized);
    } catch (err) {
      setCategories(previousCategories);
      setOrderError(err.response?.data?.error || 'Failed to update the category order.');
    } finally {
      setReordering(false);
    }
  }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">{categories.length} categories in the catalogue</p>
        </div>
      ...
      {!loading && categories.length > 0 && (
        <div className="card p-3 sm:p-4 flex items-start gap-3 bg-blue-50/50 border-blue-100">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-700">User app listing order</p>
            <p className="text-xs text-slate-500 mt-0.5">
              Use the arrows to move categories. The top category appears first in the user app.
            </p>
          </div>
        </div>
      )}
              ...
              <div key={category.id} className="card p-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold text-slate-800 capitalize truncate">{category.label}</p>
                    <code className="text-[11px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded-lg inline-block max-w-full truncate">
                      {category.slug}
                    </code>
                  </div>
                ...
                <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                  <span className="text-xs font-semibold text-slate-500">App order #{index + 1}</span>
                  <PriorityControls
                    index={index}
...
function PriorityControls({ index, total, disabled, onMove }) {
  return (
    <div className="flex items-center gap-1">
      <button
        type="button"
        ...
        title="Move earlier in app"
        aria-label="Move category earlier in app"
      >
        <ChevronUp size={15} />
      </button>
      <button
        ...
        className="btn-icon p-2 disabled:opacity-30 disabled:cursor-not-allowed"
        title="Move later in app"
        aria-label="Move category later in app"
      >
        <ChevronDown size={15} />
      </button>
    </div>
